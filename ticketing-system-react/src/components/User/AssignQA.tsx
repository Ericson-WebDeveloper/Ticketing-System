...
const AssignQA = ({ show, setModal, title, qas, ticket }: AssignQAProps) => {
  const [selectQA, setQA] = useState(0);
  const { returningStatusColor } = OtherHelper();
  const dispatch = useAppDispatch();

  const setNewQAHandler = async () => {
    try {
      dispatch(SET_PROG_LOADING(true));
      const data = {
        user_id: ticket?.qa_id || 0,
      };
      let response = await assignToQa(data, ticket?.id!);
